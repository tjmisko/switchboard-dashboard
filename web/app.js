...
let lastPlanText = "";
let lastSummaries = {};    // /api/summaries session_id → {name, description, summary}
...
// loadSummaries fetches the archival session summaries (name / description /
// narrative generated by session-digest). Tooltips and popouts read
// lastSummaries lazily at hover/click time, so no repaint is needed when the
// map refreshes. Best-effort: an empty or failed fetch just leaves hovers
// unenriched.
...
function nameSegTipHTML(lane, seg) {
  ...
  return tipHead(`${escapeHTML(seg.label || "(unnamed)")}${note}`, null,
    ...
    + (sum && sum.summary ? `<div class="t-hint">click for the session summary</div>` : "");
}

// sessionPopoutHTML is the pinned card for a session bar: the generated
// archival identity (name, one-liner, narrative) from session-digest, plus the
// lane's own identity footer. Only wired when a summary exists for the lane.
function sessionPopoutHTML(lane) {
  ...
  return `<div class="po-head">${escapeHTML(sum.name || currentName(lane) || "(unnamed)")}</div>`
    ...
    + (sum.summary ? `<div class="po-summary">${escapeHTML(sum.summary)}</div>` : "")
